...
import { getAdminTrips, getAdminTrip } from '@/lib/adminApi';
...
function DetailModal({ trip, onClose }) {
  if (!trip) return null;
  const t = trip;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div>
            <h2 className="text-lg font-semibold text-slate-800">Détails de la course</h2>
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-slate-700">{t.pickup_address || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-0.5">Destination</p>
                <p className="text-sm text-slate-700">{t.dropoff_address || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-0.5">Distance</p>
                <p className="text-sm text-slate-700">{t.distance_km ? `${parseFloat(t.distance_km).toFixed(1)} km` : '—'}</p>
              </div>
              <div>
                <p className="text-xs text-slate-400 mb-0.5">Durée estimée</p>
                <p className="text-sm text-slate-700">{t.estimated_duration ? `${t.estimated_duration} min` : '—'}</p>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 rounded-lg p-3">
                ...
                <p className="text-sm font-medium text-slate-800">{t.client?.name || t.client_name || '—'}</p>
                <p className="text-xs text-slate-500">{t.client?.phone || t.client_phone || ''}</p>
              </div>
              <div className="bg-slate-50 rounded-lg p-3">
                <p className="text-xs text-slate-400 mb-1">Chauffeur</p>
                <p className="text-sm font-medium text-slate-800">{t.driver?.name || t.driver_name || 'Non assigné'}</p>
                <p className="text-xs text-slate-500">{t.driver?.phone || t.driver_phone || ''}</p>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                ...
                <span className="text-sm font-medium text-slate-700">{formatFC(t.estimated_fare)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-sm text-slate-500">Tarif final</span>
                <span className="text-sm font-medium text-slate-700">{formatFC(t.final_fare)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-sm text-slate-500">Commission</span>
                <span className="text-sm font-medium text-orange-600">{formatFC(t.commission)}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-slate-100">
                <span className="text-sm text-slate-500">Gains chauffeur</span>
                <span className="text-sm font-medium text-green-600">{formatFC(t.driver_earnings)}</span>
              </div>
            </div>
            <div className="mt-3 flex items-center gap-2">
              <span className="text-xs text-slate-400">Paiement :</span>
              <span className="text-xs font-medium text-slate-700 bg-slate-100 px-2 py-0.5 rounded">
                {t.payment_method === 'mobile' ? 'Mobile Money' : 'Espèces'}
              </span>
              {t.payment_status && (
                <span className={`text-xs font-medium px-2 py-0.5 rounded ${t.payment_status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                  {t.payment_status === 'paid' ? 'Payé' : t.payment_status}
                </span>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ['Demandé', t.created_at],
                ['Accepté', t.accepted_at],
                ['Démarré', t.started_at],
                ['Terminé', t.completed_at || t.cancelled_at],
              ].map(([label, val]) => (
                <div key={label}>
...
export default function CoursesPage() {
  ...
  // Stats computed from current full dataset
  ...
  // Fetch stats separately (no filters, today)
  useEffect(() => {
    const today = new Date().toISOString().slice(0, 10);
    Promise.all([
      getAdminTrips({ limit: 1 }),
      getAdminTrips({ status: 'ongoing,pickup,accepted', limit: 1 }),
      getAdminTrips({ status: 'completed', date_from: today, date_to: today, limit: 1 }),
      getAdminTrips({ status: 'cancelled', date_from: today, date_to: today, limit: 1 }),
    ]).then(([all, ongoingRes, completedRes, cancelledRes]) => {
      setStats({
        total: all.data.total ?? all.data.count ?? 0,
        ongoing: ongoingRes.data.total ?? ongoingRes.data.count ?? 0,
        completed: completedRes.data.total ?? completedRes.data.count ?? 0,
  ...
  const openDetail = async (id) => {
    ...
    setSelectedTrip({ id });
    try {
      const res = await getAdminTrip(id);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total courses" value={stats.total.toLocaleString('fr-FR')} />
        <StatCard label="En cours" value={stats.ongoing.toLocaleString('fr-FR')} color="text-blue-600" />
        <StatCard label="Complétées aujourd'hui" value={stats.completed.toLocaleString('fr-FR')} color="text-green-600" />
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Nom client, chauffeur ou ID..."
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="font-semibold text-slate-800">
            Courses{total > 0 ? ` — ${total.toLocaleString('fr-FR')} résultats` : ''}
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-slate-400 text-sm">Aucune course trouvée.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  {['ID', 'Client', 'Chauffeur', 'Départ → Destination', 'Distance', 'Tarif', 'Paiement', 'Statut', 'Date', 'Actions'].map((h) => (
              ...
              <tbody className="divide-y divide-slate-100">
                {trips.map((trip) => (
                  <tr key={trip.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-slate-500">{String(trip.id).slice(0, 8)}</td>
                    <td className="px-4 py-3 font-medium text-slate-800 whitespace-nowrap">
                      {trip.client?.name || trip.client_name || '—'}
                    </td>
                    <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                      {trip.driver?.name || trip.driver_name || <span className="text-slate-400 italic">Non assigné</span>}
                    </td>
                    <td className="px-4 py-3 max-w-[220px]">
                      <span className="text-slate-700">{truncate(trip.pickup_address, 20)}</span>
                      <span className="text-slate-400 mx-1">→</span>
                      <span className="text-slate-700">{truncate(trip.dropoff_address, 20)}</span>
                    </td>
                    <td className="px-4 py-3 text-slate-600 whitespace-nowrap">
                    ...
                    <td className="px-4 py-3 text-slate-800 font-medium whitespace-nowrap">
                      {formatFC(trip.final_fare || trip.estimated_fare)}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${trip.payment_method === 'mobile' ? 'bg-purple-100 text-purple-700' : 'bg-slate-100 text-slate-600'}`}>
                        {trip.payment_method === 'mobile' ? 'Mobile' : 'Espèces'}
                      </span>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap">
      ...
      {(selectedTrip || modalLoading) && (
        modalLoading ? (
          ...
          <DetailModal trip={selectedTrip} onClose={() => setSelectedTrip(null)} />
        )
      )}
    </div>
  );
