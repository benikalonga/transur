...
import { ChevronRight, Clock, HelpCircle, LogOut, Package, Pencil, Smartphone, User, Wallet } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f0f4f8] flex flex-col">
      ...
      <div className="header-red relative px-6 pt-14 pb-24 text-white overflow-hidden">
        ...
        <div className="relative flex items-center gap-4">
          <div className="w-[72px] h-[72px] rounded-2xl overflow-hidden border-2 border-white/20 flex-shrink-0 bg-white/15 flex items-center justify-center text-2xl font-black">
            {user.photo_url
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-sm mt-0.5">
              ...
              <span className="text-yellow-300">★ {parseFloat(user.agent_rating || 5).toFixed(1)}</span>
              <span className="text-red-200 text-xs">({user.total_deliveries || 0} livraisons)</span>
            </div>
            <p className="text-red-100 text-xs mt-0.5">Livreur · Transur</p>
          </div>
        </div>
      ...
      <div className="flex-1 px-5 py-5 -mt-2 space-y-4">
        ...
        <div className="card">
          <div className="flex items-center justify-between p-4 border-b border-gray-100">
            ...
            <button onClick={() => setEditing(!editing)} className="text-[#CE1126] text-sm font-bold flex items-center gap-1">
              <Pencil size={13} /> {editing ? 'Annuler' : 'Modifier'}
            </button>
          </div>
          <div className="p-4 space-y-4">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900 flex items-center gap-1.5">
                <Smartphone size={14} className="text-gray-400" /> {user.phone}
              </p>
            </div>
            <div>
              <label className="text-xs text-gray-400 block mb-1">Moyen de transport</label>
              <p className="font-semibold text-gray-900 capitalize">{transportIcon} {user.transport_type}</p>
            </div>
          </div>
          {editing && (
            <div className="px-4 pb-4">
              <button onClick={handleSave} disabled={saving} className="btn-primary btn-red">
        ...
        <div className="card divide-y divide-gray-100">
          <Link href="/delivery/wallet" className="flex items-center justify-between p-4">
            <span className="flex items-center gap-3">
              <span className="w-8 h-8 bg-red-50 rounded-xl flex items-center justify-center">
                <Wallet size={16} className="text-[#CE1126]" />
              </span>
              <span className="font-semibold text-gray-700">Portefeuille</span>
            </span>
            <ChevronRight size={16} className="text-gray-300" />
          </Link>
          <Link href="/delivery/history" className="flex items-center justify-between p-4">
            <span className="flex items-center gap-3">
              <span className="w-8 h-8 bg-gray-50 rounded-xl flex items-center justify-center">
                <Clock size={16} className="text-gray-500" />
              </span>
              <span className="font-semibold text-gray-700">Historique des livraisons</span>
            </span>
            <ChevronRight size={16} className="text-gray-300" />
          </Link>
      ...
      <nav className="bottom-nav px-6 pt-3 pb-safe flex justify-around">
        <Link href="/delivery" className="flex flex-col items-center gap-1">
        ...
        <button className="flex flex-col items-center gap-1">
          ...
          <span className="text-[10px] font-bold text-[#CE1126]">Profil</span>
        </button>
      </nav>
    </div>
  );
