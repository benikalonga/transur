...
import { Wallet, BarChart2, CreditCard, Gift, AlertTriangle } from 'lucide-react';
import formatCDF from '@/lib/currency';

const PROVIDERS = [
  { id: 'mpesa',        label: 'M-Pesa',        emoji: '📱', color: 'bg-[#CE1126]' },
  { id: 'airtel_money', label: 'Airtel Money',   emoji: '📡', color: 'bg-red-600'   },
  { id: 'orange_money', label: 'Orange Money',   emoji: '🟠', color: 'bg-orange-500'},
  { id: 'vodacom',      label: 'Vodacom',        emoji: '📶', color: 'bg-red-700'   },
];
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f8] flex flex-col">
      ...
      <div className="header-dark relative px-6 pt-14 pb-24 text-white overflow-hidden">
        ...
        {/* Balance card */}
        <div className={`relative rounded-2xl p-5 ${wallet?.is_blocked ? 'bg-red-900/60 border border-red-700/50' : 'bg-white/10 backdrop-blur'}`}>
          <p className="text-gray-300 text-xs mb-1">Solde actuel</p>
      ...
      <div className="flex-1 px-5 py-5 -mt-2 space-y-4">
        ...
        {/* ── Recharge button ── */}
        <button onClick={() => setShowRecharge(!showRecharge)} className="btn-primary flex items-center justify-center gap-2">
        ...
        {showRecharge && (
          <form onSubmit={handleRecharge} className="card p-5 space-y-4 slide-up">
              ...
              {PROVIDERS.map((p) => (
                <button key={p.id} type="button" onClick={() => setForm({ ...form, provider: p.id })}
                  className={`py-3 rounded-2xl text-white text-sm font-bold transition-all flex items-center justify-center gap-2 ${p.color} ${
                    form.provider === p.id ? 'ring-2 ring-offset-2 ring-[#007DC5] opacity-100' : 'opacity-60'
                  }`}>
            ...
            <div className="bg-blue-50 border border-blue-100 p-3 rounded-2xl text-xs text-blue-700 leading-relaxed">
            ...
            <button type="submit" disabled={recharging} className="btn-primary">
              {recharging
        ...
        <div>
          ...
          {transactions.length === 0 ? (
            <div className="card p-8 text-center fade-in">
              <div className="flex justify-center mb-2"><BarChart2 size={48} className="text-gray-300" /></div>
              <p className="text-gray-400 text-sm">Aucune transaction</p>
            </div>
          ) : (
            <div className="space-y-2">
              {transactions.map((tx, i) => {
                const info = TX_ICONS[tx.type] || { label: tx.type, Icon: Wallet, positive: tx.amount > 0 };
                ...
                return (
                  <div key={tx.id} className="card p-4 flex items-center gap-3 slide-up" style={{animationDelay:`${i*40}ms`}}>
                    <div className={`w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 ${
                      info.positive ? 'bg-green-50' : 'bg-red-50'
                    }`}>
                      <TxIcon size={20} className={info.positive ? 'text-green-600' : 'text-[#CE1126]'} />
                    </div>
